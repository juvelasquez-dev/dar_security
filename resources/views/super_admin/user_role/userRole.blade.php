    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .sidebar-link-badge {
            ...
            background: var(--green-100); color: var(--green-700); padding: 2px 7px; border-radius: 20px;
        }
        ...
        .header-date-chip {
            ...
            padding: 6px 14px; font-size: 0.78rem; color: var(--gray-600);
            display: flex; align-items: center; gap: 6px; box-shadow: var(--shadow-sm); white-space: nowrap;
        }

        /* ─── Stat Cards ────────────────────────────────────── */
        .stat-card {
            background: #fff; border-radius: var(--radius); box-shadow: var(--shadow-sm);
            padding: 1.2rem 1.3rem; display: flex; align-items: center; gap: 0.9rem;
            transition: box-shadow 0.22s, transform 0.22s; border: 1px solid var(--gray-200); height: 100%;
        }
        ...
        .stat-value { font-size: 1.7rem; font-weight: 700; color: var(--text-main); line-height: 1; margin-bottom: 3px; }
        .stat-label { font-size: 0.78rem; font-weight: 500; color: var(--text-muted); margin: 0; }

        /* ─── Filter Card ───────────────────────────────────── */
        .filter-card {
            background: #fff; border-radius: var(--radius);
            box-shadow: var(--shadow-sm); border: 1px solid var(--gray-200); padding: 1.2rem 1.4rem;
        }
        ...
        .user-table tbody tr { border-bottom: 1px solid var(--gray-100); transition: background 0.14s; }
        .user-table tbody tr:last-child { border-bottom: none; }
        .user-table tbody tr:hover { background: var(--gray-50); }
        .user-table td { padding: 0.75rem 1.1rem; vertical-align: middle; color: var(--text-main); }
        .cell-name { font-weight: 600; color: var(--green-800); }
        .cell-name small { display: block; font-weight: 400; font-size: 0.72rem; color: var(--text-muted); margin-top: 1px; }

        /* ─── Role Badges ───────────────────────────────────── */
        .role-badge {
            display: inline-flex; align-items: center; gap: 5px;
            padding: 3px 10px; border-radius: 20px; font-size: 0.71rem; font-weight: 600; white-space: nowrap;
        }
        ...
        .actions-menu-btn {
            ...
            color: var(--gray-600); font-size: 1rem; cursor: pointer;
            transition: all 0.16s; padding: 0;
        }
        .actions-menu-btn::after { display: none !important; }
        .actions-menu-btn:hover { background: var(--gray-100); border-color: var(--gray-400); color: var(--text-main); }
        .actions-menu-btn:focus { outline: none; box-shadow: 0 0 0 3px rgba(31,128,60,0.15); border-color: var(--green-600); }
        .actions-dropdown-menu {
            border: 1px solid var(--gray-200); border-radius: 10px;
            box-shadow: 0 8px 24px rgba(13,59,30,0.13); padding: 4px; min-width: 165px; font-size: 0.83rem;
        }
        ...
        .actions-dropdown-menu .dropdown-item i { font-size: 0.85rem; width: 16px; text-align: center; flex-shrink: 0; }
        ...
        .actions-dropdown-menu .dropdown-item.item-edit:hover   { background: #e8f0fe; color: #1a73e8; }
        ...
        .actions-dropdown-menu .dropdown-divider { margin: 4px 0; border-color: var(--gray-200); }

        /* ─── Empty State ───────────────────────────────────── */
        .empty-state { text-align: center; padding: 3rem 1rem; color: var(--text-muted); }
        .empty-state i { font-size: 2.5rem; color: var(--gray-400); margin-bottom: 0.75rem; display: block; }
        ...
        .side-card-header { padding: 1.1rem 1.5rem 0.9rem; border-bottom: 1px solid var(--gray-200); }
        .side-card-title { font-size: 0.9rem; font-weight: 700; color: var(--text-main); margin: 0; display: flex; align-items: center; gap: 0.45rem; }
        ...
        .activity-item {
            ...
            border-bottom: 1px solid var(--gray-100); align-items: flex-start; transition: background 0.14s;
        }
        .activity-item:last-child { border-bottom: none; }
        .activity-item:hover { background: var(--gray-50); }
        .activity-dot {
            width: 32px; height: 32px; border-radius: 50%;
            display: flex; align-items: center; justify-content: center; font-size: 0.85rem; flex-shrink: 0; margin-top: 1px;
        }
        ...
        .form-control, .form-select { font-size: 0.86rem; border-color: var(--gray-200); transition: border-color 0.18s, box-shadow 0.18s; }
    ...
    <div class="navbar-right">
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0" style="min-width:280px; border-radius:12px; margin-top:8px;">
                ...
                <li><a class="dropdown-item py-2" href="#" style="font-size:.82rem;"><i class="bi bi-building text-primary me-2"></i> Branch updated<div class="text-muted" style="font-size:.72rem; padding-left:1.4rem;">5 hours ago</div></a></li>
                <li><a class="dropdown-item py-2" href="#" style="font-size:.82rem;"><i class="bi bi-cart-check text-warning me-2"></i> Order completed<div class="text-muted" style="font-size:.72rem; padding-left:1.4rem;">Yesterday</div></a></li>
...
<aside class="sidebar" id="mainSidebar">
    <div class="sidebar-inner">
        ...
        <a href="{{ url('/roles') }}" class="sidebar-link active"><i class="bi bi-person-badge"></i> User Roles</a>
        <a href="{{ url('/branches') }}" class="sidebar-link"><i class="bi bi-building"></i> PBD Management <span class="sidebar-link-badge">8</span></a>
        ...
        <a href="{{ url('/reports') }}" class="sidebar-link"><i class="bi bi-bar-chart-line"></i> Sales Report</a>
...
<main class="page-wrapper">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-header-sub">Manage system users, assigned roles, and access levels across E-Agraryo Merkado.</p>
        </div>
        <div class="header-actions">
            <button type="button" class="btn fw-semibold d-flex align-items-center gap-2"
                    data-bs-toggle="modal" data-bs-target="#addUserModal"
                    style="background:var(--green-600); color:#fff; border-radius:10px; border:none; font-size:.84rem;">
                <i class="bi bi-person-plus-fill"></i> Add User
            </button>
            <a href="#" class="btn fw-semibold d-flex align-items-center gap-2"
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-green"><i class="bi bi-people-fill"></i></div><div><div class="stat-value">{{ $totalUsers ?? 0 }}</div><p class="stat-label">Total Users</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-gold"><i class="bi bi-shield-lock-fill"></i></div><div><div class="stat-value">{{ $totalSuperAdmins ?? 0 }}</div><p class="stat-label">Super Admins</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-blue"><i class="bi bi-building-fill-gear"></i></div><div><div class="stat-value">{{ $totalCarposAdmins ?? 0 }}</div><p class="stat-label">Admin CARPOS</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-teal"><i class="bi bi-person-badge-fill"></i></div><div><div class="stat-value">{{ $totalArboAdmins ?? 0 }}</div><p class="stat-label">ARBO Admins</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-gold"><i class="bi bi-wallet2"></i></div><div><div class="stat-value">{{ $totalFinance ?? 0 }}</div><p class="stat-label">Finance</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-green"><i class="bi bi-shop"></i></div><div><div class="stat-value">{{ $totalSellers ?? 0 }}</div><p class="stat-label">Sellers</p></div></div>
        </div>
        <div class="col-6 col-md-4 col-xl-2">
            <div class="stat-card"><div class="stat-icon-wrap stat-icon-red"><i class="bi bi-bag-check-fill"></i></div><div><div class="stat-value">{{ $totalBuyers ?? 0 }}</div><p class="stat-label">Buyers</p></div></div>
        </div>
    </div>
        ...
        <form method="GET" action="{{ url()->current() }}">
            <div class="row g-3 align-items-end">
                <div class="col-md-4">
                    ...
                    <input id="userSearch" type="text" name="search" class="form-control"
                ...
                <div class="col-md-2 d-flex gap-2">
                    <button type="submit" class="btn btn-success flex-fill" style="border-radius:8px;"><i class="bi bi-search"></i></button>
                    <a href="{{ route('super_admin.user_roles.index') }}" class="btn btn-outline-secondary flex-fill" style="border-radius:8px;"><i class="bi bi-arrow-counterclockwise"></i></a>
                </div>
            </div>
        </form>
    </div>
        ...
        <div class="col-xl-8">
            <div class="table-card">
                <div class="table-card-header">
                    <h6 class="table-card-title"><i class="bi bi-people-fill"></i> System Users</h6>
                </div>
                    ...
                    <table class="user-table">
                        ...
                        @forelse($users as $user)
                            ...
                            <tr>
                                ...
                                <td style="font-size:.82rem; color:var(--text-muted);">{{ $user->province->name ?? '—' }}</td>
                                <td>
                                            ...
                                            <li>
                                                @if($user->status === 'active')
                                                    <button type="button" class="dropdown-item item-deactivate"
                                                            ...
                                                            data-id="{{ $user->id }}" data-name="{{ $fullName }}" data-action="deactivate">
                                                        <i class="bi bi-slash-circle-fill"></i> Deactivate
                                                    </button>
                                                @else
                                                    <button type="button" class="dropdown-item item-activate"
                                                            data-bs-toggle="modal" data-bs-target="#statusModal"
                                                            data-id="{{ $user->id }}" data-name="{{ $fullName }}" data-action="activate">
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <button type="button" class="btn btn-sm" data-bs-toggle="modal" data-bs-target="#addUserModal"
            ...
            <form method="POST" action="{{ route('super_admin.user_roles.store') }}" enctype="multipart/form-data">
                ...
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="addUserModalLabel">
                        <i class="bi bi-person-plus-fill me-2" style="color:var(--green-600);"></i>Add User
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-4"><label class="form-label">First Name</label><input type="text" name="first_name" class="form-control" required style="border-radius:8px;" value="{{ old('first_name') }}"></div>
                        <div class="col-md-4"><label class="form-label">Middle Name</label><input type="text" name="middle_name" class="form-control" style="border-radius:8px;" value="{{ old('middle_name') }}"></div>
                        <div class="col-md-4"><label class="form-label">Last Name</label><input type="text" name="last_name" class="form-control" required style="border-radius:8px;" value="{{ old('last_name') }}"></div>
                        <div class="col-md-6"><label class="form-label">Email</label><input type="email" name="email" class="form-control" style="border-radius:8px;" value="{{ old('email') }}"></div>
                        <div class="col-md-6"><label class="form-label">Contact Number</label><input type="text" name="contact_number" class="form-control" style="border-radius:8px;" value="{{ old('contact_number') }}"></div>
                        <div class="col-md-6"><label class="form-label">Username</label><input type="text" name="username" class="form-control" required style="border-radius:8px;" value="{{ old('username') }}"></div>
                        <div class="col-md-6"><label class="form-label">Avatar</label><input type="file" name="avatar" accept="image/*" class="form-control" style="border-radius:8px;"></div>
                        <div class="col-md-6">
                            <label class="form-label">Role</label>
            ...
            <form id="statusForm" method="POST" action="#">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning mb-0" style="border-radius:10px; font-size:.84rem;">
                        <i class="bi bi-exclamation-triangle-fill me-2"></i>This action will affect the user's access to the system.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal" style="border-radius:8px;">Cancel</button>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const wrapper = document.getElementById('pbdRegionWrapper');
    const region = document.getElementById('pbdRegionSelect');
    function toggleRegion(){
        if(!role || !wrapper) return;
        if(role.value){ wrapper.style.display = ''; if(region) region.setAttribute('required','required'); }
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function applyFilters(){
        ...
        tbody.querySelectorAll('tr').forEach(function(row){
            ...
            row.style.display = ((q === '' || text.indexOf(q) !== -1) && (roleVal === '' || rowRole.indexOf(roleVal) !== -1) && (statusVal === '' || rowStatus.indexOf(statusVal) !== -1)) ? '' : 'none';
        });
    }
...
<div id="branchesPanel" aria-hidden="true">
    <div id="branchesPanelHeader"><strong>Branches</strong><button id="branchesPanelClose" class="btn btn-sm btn-outline-secondary">Close</button></div>
...
document.addEventListener('click', function(e){
    ...
    if(!btn) return;
    e.preventDefault();
    openBranchesPanel(btn.getAttribute('data-href') || btn.getAttribute('href'));
});
function openBranchesPanel(url){
    ...
    fetch(url, {headers:{'X-Requested-With':'XMLHttpRequest'}})
        .then(r=>r.text()).then(html=>{ var m = html.match(/<main[\s\S]*?>[\s\S]*?<\/main>/i); content.innerHTML = m ? m[0] : html; })
